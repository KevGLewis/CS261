...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    // FIXME: Implement
    clearVisited(graph);
    Deque* knownReachVert = dequeNew();
    dequePushFront(knownReachVert, source);
    
    Vertex* theOne;
    
    while(!dequeIsEmpty(knownReachVert)){
        theOne = dequeFront(knownReachVert);
        dequePopFront(knownReachVert);
        
        if(!theOne->isVisited){
            theOne->isVisited = 1;
            for (int i = 0; i < theOne->numNeighbors; ++i)
            {
                Vertex* neighbor = theOne->neighbors[i];
                if (!neighbor->isVisited)
                {
                    dequePushFront(knownReachVert, neighbor);
                }
            }
        }
        
        if(destination->isVisited){
            dequeDelete(knownReachVert);
            return 1;
        }
    }
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while(!dequeIsEmpty(knownReachVert)){
        theOne = dequeBack(knownReachVert);
        dequePopBack(knownReachVert);

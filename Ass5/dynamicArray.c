...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    int smallChild = position;
    
    
    if(rightChild < last){
        //Now determine the smallest child
        ...
        if(compare(dyGet(heap, smallChild), dyGet(heap, position)) == -1){
            dySwap(heap, smallChild, position);
...
{
    // FIXME: implement
    dyAdd(heap, value); //Adds value to end of heap
...
void dyHeapSort(DynamicArray* heap, compareFunction compare)
{
    ...
    for(int last = dySize(heap) - 1; last > 0; last--){
        dySwap(heap, 0, last);

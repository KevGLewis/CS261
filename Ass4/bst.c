...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    // Run through the tree until you hit the value or null
    while(itr != NULL){
        
        if(compare(itr->val, val) == -1){
            itr = itr->right;
        }
        
        else if(compare(itr->val, val) == 1){
            itr = itr->left;
        }
...
struct Node *_removeLeftMost(struct Node *cur)
{
    /*write this*/
    if(cur->left == NULL){
        struct Node* returnNode = cur->right;
        free(cur);
        cur = 0;
        return returnNode;
    }
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if(compare(cur->val, val) == 0){
        if(cur->right == NULL){
            struct Node* returnNode = cur->left;
            free(cur);
            cur = 0;
            return returnNode;
        }
